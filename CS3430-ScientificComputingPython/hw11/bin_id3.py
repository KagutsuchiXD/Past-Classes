...
class bin_id3(object):
    ...
    def find_best_attribute(examples, target_attrib, attribs, avt):
        ...
        """
        table = {}
        best_attrib = ('nothing', 0)
        for a in attribs:
            if a is not target_attrib:
                g = bin_id3.gain(examples, target_attrib, a, avt)
                table[a] = g
                if g > best_attrib[1]:
                    best_attrib = (a, g)
        assert best_attrib[0] != 'nothing'
        return best_attrib[0], best_attrib[1], table
    ...
    def predict(node, example):
        ...
        global PLUS
        global MINUS
        if node.get_label() is PLUS:
            return PLUS
        if node.get_label() is MINUS:
            return MINUS
        rat = node.get_label()
        if rat not in example.keys():
            return MINUS
        rav = bin_id3.get_example_attrib_val(example, rat)
        if rav not in node.get_children():
            return MINUS
        ch = node.get_child(rav)
