...
def correlate_windows(window_a, window_b, corr_method='fft', nfftx=None, nffty=None):
    ...
    if corr_method == 'fft':
        ...
        return convolve2d(normalize_intensity(window_a),normalize_intensity(window_b[::-1, ::-1]), 'full')
    else:
...
def extended_search_area_piv(
    ...
    for k in range(n_rows):
        ...
        for m in range(n_cols):
            ...
            print('window_b:  il={} : ir={} ------- jt={} : jb={}'.format(il,ir,jt,jb))
            ...
            print('window_a:  il={} : ir={} ------- jt={} : jb={}'.format(il, ir, jt, jb))
            if np.any(window_a):
                ### THIS IS THE NATIVE CALL with FFT
                corr = correlate_windows(window_a, window_b,corr_method=corr_method,nfftx=nfftx, nffty=nffty)

                ### THIS IS OUR IMPLEMENTATTION, slow but more flexible.
                #corr = directCorrelation(window_a, window_b)
...
### THIS IS OUR IMPLEMENTATION HADAMARD PRODUCT BASED
### IMPLEMENTATION.
def directCorrelation(image1, image2):
    corr = {}
    P, Q = image1.shape
    M, N = image2.shape
    C = []
    for r in range(-(M - 1), P, 1):
        C_SUB = []
        for s in range(-(N - 1), Q, 1):
                ...
                for j in range(image2.shape[0]):
                    if i + r >= 0 and j + s >= 0:
                        if (r, s) not in corr:
                            corr[(r, s)] = 0
                        try:
                            corr[(r, s)] = corr[(r, s)] + (image2[i, j] * image1[i + r, j + s])
                        except:
                            corr[(r, s)] = corr[(r, s)] + 0
                    else:
                        if (r, s) not in corr:
